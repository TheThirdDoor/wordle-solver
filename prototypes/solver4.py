...
def strategy_weighted_buckets(guess: str, possible_answers: list[str]):
    ...
    score = total_buckets + (ones_count * 1.5)
...
def get_best_guess(possible_answers: list[str]) -> str:
    ...
    if n <= strategy2cutoff:
        retval = strategy_recursive_exact(possible_answers)
        print(f"Endgame solver chose: {retval} for {possible_answers}")
        return retval
